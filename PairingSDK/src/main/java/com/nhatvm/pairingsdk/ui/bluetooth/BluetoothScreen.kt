    ...
    Column(
        ...
        when (pairingState) {
            PairingState.Default -> {
                Button(onClick = { pairingViewModel.startPairing()}, contentPadding = PaddingValues(horizontal = 48.dp, vertical = 12.dp)) {
                    Text("Start Pairing")
                }
            }
            PairingState.ScanningBluetooth -> {
                Text(text = "Scanning bluetooth ...", style = TextStyle(color = Color.Black, fontSize = 16.sp))
            }
            else -> {
